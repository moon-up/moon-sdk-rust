...
/// struct for typed errors of method [`sign_broadcast_user_op_tx`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SignBroadcastUserOpTxError {
    UnknownValue(serde_json::Value),
}
...
pub async fn get_address(configuration: &configuration::Configuration, authorization: &str, account_name: &str, input_body: crate::models::InputBody) -> Result<crate::models::AccountControllerResponse, Error<GetAddressError>> {
    ...
    let local_var_uri_str = format!("{}/erc4337/{accountName}/address", local_var_configuration.base_path, accountName=crate::apis::urlencode(account_name));
...
pub async fn sign_broadcast_user_op_tx(configuration: &configuration::Configuration, authorization: &str, account_name: &str, input_body: crate::models::InputBody) -> Result<crate::models::AccountControllerResponse, Error<SignBroadcastUserOpTxError>> {
    ...
    let local_var_uri_str = format!("{}/erc4337/{accountName}/sign-broadcast-userop-tx", local_var_configuration.base_path, accountName=crate::apis::urlencode(account_name));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<SignBroadcastUserOpTxError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
